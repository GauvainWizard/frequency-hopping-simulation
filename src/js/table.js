...
class GsmSystem {
    ...
    constructor(numTRX, maiCount, hsn = 0, lambda, mu) {
        this.lambda = lambda;
        this.mu = mu;
        this.TRXs = Array.from({ length: numTRX }, (v, i) => new TRX(i, hsn, maiCount)).sort(() => Math.random() - 0.5); // Create an array of TRXs and shuffle it to simulate random distribution of maio
    }
    simulate() {
        let frame = 1;
        const interval = setInterval(() => {
            ...
            this.TRXs.forEach((trx, index) => {
                ...
                if (!trx.is_available()) {
                    if (trx_td) {
                        trx_td.innerHTML += `<td style="background-color:hsl(${(trx.get_frequency() * 137.508) % 360}, 80%, 60%); border-top: 2px solid black; border-bottom: 2px solid black;" id="trx${index + 1}_slot${frame}">${trx.get_frequency().toString()}</td>`;
                    }
                    let trx_slot = document.querySelector(`#trx${index + 1}_slot${frame}`);
...
createButton?.addEventListener("click", () => {
    if (trxCountInput && simulatorTable && hsn && lambda && mu) {
        ...
        const gsmSystem = new GsmSystem(trxCount, maiCount, hsnValue, lambdaValue, muValue);
